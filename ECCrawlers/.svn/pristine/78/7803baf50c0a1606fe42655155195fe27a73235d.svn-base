...
public class AmazonCNSKUCrawlers {
	final static Logger logger = Logger.getLogger(AmazonCNSKUCrawlers.class);
	public static void main (String [] args){
		AmazonCNSKUCrawlers amzonCrawler = new AmazonCNSKUCrawlers();
		ECDAO ecdao = null;
		try{
			ecdao = new ECDAO();
		...
		try{
			ArrayList <ECSKU> skuList = amzonCrawler.crawlData("Estee Lauder", 100L);
	...
	public ArrayList<ECSKU> crawlData(String keyword,long channelId) throws Exception{
		...
		for(int productFlip =1;productFlip<=20;productFlip++){
			Document doc = Jsoup.connect("https://www.amazon.cn/s/page="+productFlip+"&keywords="+keyword+"&ie=UTF8").ignoreContentType(true).ignoreHttpErrors(true).userAgent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.75.14 (KHTML, like Gecko) Version/7.0.3 Safari/7046A194A").referrer("http://www.yahoo.com").timeout(10000).get();
			logger.debug("https://www.amazon.cn/s/page="+productFlip+"&keywords="+keyword+"&ie=UTF8");
			...
			for (int i = 0; i < li.size(); i++) {
				...
				try{
					String url  = "https://www.amazon.cn/dp/"+li.get(i).attr("data-asin");
